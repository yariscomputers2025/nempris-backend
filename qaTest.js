...
const run = async () => {
  ...
  const regSeller = await makeRequest('http://localhost:5000/api/auth/register', {
    ...
    body: JSON.stringify({ name: 'Seller QA', email: sellerEmail, password: 'Password1!', role: 'seller' })
  });
  ...
  const regUser = await makeRequest('http://localhost:5000/api/auth/register', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ name: 'Regular QA', email: userEmail, password: 'Password1!', role: 'user' })
  });
